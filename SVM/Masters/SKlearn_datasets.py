...
if __name__ == '__main__':
    ...
    save_directory = 'C:/Users/Галина Андреевна/Desktop/выборки/disp/'
    ...
    for ds_id, data_set in enumerate(datasets):
        ...
        informing = False
        # informing = True
        left_x, right_x = np.min(data_set[:-1])-1, np.max(data_set[:-1])+1
        ...
        # clf = SVC(C=C, kernel='linear', tol=eps, max_iter=max_its)
        ...
        plt.contour(x1, x2, Z + 1, [0.0], colors='y', linewidths=1, origin='lower')
        plt.contour(x1, x2, Z - 1, [0.0], colors='y', linewidths=1, origin='lower')
        # plt.title('n = {}, C = {}, lambda = {}, ds_id = {}\n sum_acc = {}, max_acc = {}'.format(
        #     l, model.C, model.lmbd, ds_id, np.round(accuracy_s*100, decimals=1), np.round(accuracy_r*100, decimals=1)))
        ...
        plt.savefig(save_directory+'ds_id_{}-{}_max_acc_{}_{}.jpg'.format(
            ds_id, l, np.round(accuracy_r*100, decimals=0), "свои_данные"))
        plt.show()
