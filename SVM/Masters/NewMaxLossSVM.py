...
class MaxLossSVM(object):
    ...
    def fit(self, train_data, test_data=None, eps=1e-3, max_iters: int = 150, info=False, plotting=None):
        ...
        rs = [-self.cost_f_d1(A, self.Y, self.alpha, self.lmbd)]
        omegas = [0.0]
        Ss = [rs[-1]]
        # находим длину шага
        bettas = [self.bi_search(Ss[-1], A, self.Y)]
        # bettas = [self.bi_search(rs[-1], A, self.Y, 0, 0.5, 1e-7, 1e-8, 16)]

        # обновляем вектор альфа
        self.alpha += bettas[-1] * Ss[-1]
        ...
        # train_project = np.sum(S, axis=0).reshape((l, 1))
        # train_predict = np.sign(train_project)

        # print(self.__cost_f__(A, self.Y, self.alpha, self.lmbd, self.C))
        # print(self.calc_acc(train_predict, self.Y))
        stop_fitting = False

        while len(cost_list) < max_iters and not stop_fitting:
            # находим антиградиент
            rs.append(-self.cost_f_d1(A, self.Y, self.alpha, self.lmbd))

            # 1 вариант подсчета omega
            # omegas.append(np.dot(rs[-1].T, rs[-1]) / np.dot(rs[-2].T, rs[-2]))
            #####

            # 2 вариант
            dd_f = self.cost_f_d2(A, self.Y) * Ss[-1]
            omegas.append(np.dot(dd_f.T, rs[-1]) / np.dot((dd_f * Ss[-1]).T, Ss[-1]))
            #####

            # находим сопряженное направление
            Ss.append(rs[-1] + omegas[-1] * Ss[-1])
            # находим длину шага
            bettas.append(self.bi_search(Ss[-1], A, self.Y))
            # находим новый вектор альфа
            new_alpha = self.alpha + bettas[-1] * Ss[-1]
            # вычисляем разницу между новым векторов альфа и старым
            ...
            # НУЖНО ПОДУМАТЬ НАД ВЫЧИСЛЕНИЕМ ЦЕЛЕВОЙ ФУНКЦИИ!!!!


            # train_predict = self.predict(self.X)
            # cost_list.append(self.__cost_f__(A, self.Y, self.alpha, self.lmbd, self.C))
            ...
            train_predict = np.sign(train_project)
            cost_list.append(
                0.5 * np.sum(train_project ** 2) + (self.lmbd - 1) * np.sum(self.alpha) - self.lmbd * self.C)
            accuracy_list.append(self.calc_acc(train_predict, self.Y))

            if test_data is not None:
                S = (A_T.T * (self.Y * self.alpha).T).T
                test_project = np.sum(S, axis=0).reshape((len(Y_T), 1))
                test_predict = np.sign(test_project)
                test_cost_list.append(
                    0.5 * np.sum(test_project ** 2) + (self.lmbd - 1) * np.sum(self.alpha) - self.lmbd * self.C)
